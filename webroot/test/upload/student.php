...
<div>
	...
	<div id="lessons">
		<div class="head">
		 <span id="mylesson">我的课程</span>
			<span id="add"><a href="all_lessons.php">添加课程</span>
		</div>
		<div>
	<?php
	include_once"session_check.php";
	include_once"../connect_mysql.php";
	$username=$_SESSION['username'];
	$sql = "select 课程代码 from 选课表 where 学生用户名='$username'";  
	...
	for($length=0;$length<count($result_arr);$length++){
		...
		while($arr1=mysql_fetch_array($result1)){
			...
			echo "<a class=a href=./lesson/$result_arr[$length]/$result_arr[$length].php style=display:block;width:148px;height:148px;></a>";
